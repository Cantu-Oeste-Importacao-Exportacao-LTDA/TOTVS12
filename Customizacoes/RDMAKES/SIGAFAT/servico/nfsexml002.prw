...
User Function NFSEXml2(cCodMun,cTipo,dDtEmiss,cSerie,cNota,cClieFor,cLoja)
...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		aadd(aNota,"1")
		// Removido devido a nao existir este campo - Flavio
		...
		#IFDEF TOP
			...
			BeginSql Alias cAliasSD2
				SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF,
					...
					D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB ,D2_BASEISS
				FROM %Table:SD2% SD2
				WHERE
...
If !Empty(aNota)
	...
	cString += NFSEItem2(cCodMun,aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aDest, aNota,aTotal,aRetido,aRetServ,aDeducao,cMunPrest,cDescMunP,cNatOper)
...
cString += '<NumeroRPSSubstituido>'+AllTrim(Str(Val(aNota[07])))+'</NumeroRPSSubstituido>'
cString += '<SerieRPSSubstituido>'+AllTrim(aNota[08])+'</SerieRPSSubstituido>'
cString += '<TipoRPSSubstituido>1</TipoRPSSubstituido>'
cString += '<NfseIdSubstituido>'+aNota[08]+AllTrim(Str(Val(aNota[07])))+'</NfseIdSubstituido>'
...
//Serviço
Static Function NFSEItem2(cCodMun,aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ, aDest, aNota, aTotal,aRetido,aRetServ,aDeducao,cMunPrest,cDescMunP,cNatOper)
...
EndCase
	...
	cString += '<Tributacao>'+  Iif(Empty(aTotal[3]), "1", aTotal[3])+'</Tributacao>'
    If !Empty(cMVREGIESP)
...
For Nx := 1 to Len(aProd)   
	...
	nValLiq    := (aProd[Nx][10] * aProd[Nx][09]) - aPisXml[1] - aCofinsXml[1]  - aInssXml[1] - aIRRFXml[1] - aCSLLXml[1] - aIssRet[1]  
	...
	cXml += '<ValorTotal>'+AllTrim(ConvType2((aProd[Nx][10] * aProd[Nx][09]),15,2))+'</ValorTotal>'     
	cXml += '<BaseCalculo>'+Alltrim(ConvType2(nBaseIss ,15,2))+'</BaseCalculo>'     
	cXml += '<ISSRetido>'+Iif(!Empty(aIssRet[2]),"1","2")+'</ISSRetido>'   
	cXml += '<ValorDeducoes>'+ConvType2(aProd[Nx][21] + aProd[Nx][22],15,2)+'</ValorDeducoes>' 
