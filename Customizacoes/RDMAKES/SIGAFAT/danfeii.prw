...
//³Jean - 17/11/2012 - Variáveis de alinhamento³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ8Ù

#DEFINE PAD_LEFT            0
#DEFINE PAD_RIGHT           1
#DEFINE PAD_CENTER          2
/**CANTU-FIM**/                                             // Máximo de caracteres por linha de valores numéricos
...
//Guilherme - 19/05/16 - Indica os dados para impressao do complemento
Private cCliFor 		:= ""
Private cLojaCF 		:= ""
//Guilherme - 19/05/16 - FIm
...
/**CANTU-INICIO**/
Private lEnvPdfJob := SuperGetMv("MV_X_PDFJB",,.F.)
/**CANTU-FIM**/
...
If lExistNfe
	...
	oDanfe:Print() // Imprime 
	// Flavio - 25/11/2011
	// Caso for pdf, não imprime a segunda cópia
	if !(oDanfe:nDevice == IMP_PDF)
		oDanfe:Print() // Imprime 2a copia
	EndIf
	For _nX := 1 To Len(aNFEnv)
		If aNFEnv[_nX,5] == 'S'
			If cEmpAnt $ SuperGetMV("MV_BOLAUTO", ," ")
		  //If cEmpAnt+cFilAnt == "5001"	
				Sleep(1000)
				If _nX = 1
					oPrnTMP := TMSPrinter():New("Impressão de Boletos")
					oPrnTMP:Setup()
				Endif
				u_SelBco(aNFEnv[_nX,3],aNFEnv[_nX,4],aNFEnv[_nX,2],aNFEnv[_nX,1])
			Endif
		Endif	
	Next _nX 
	/**CANTU-FIM**/
...
/**CANTU-INICIO**/
// Customizado FVSISTEMAS - Flavio - CDR Entrega Certa
...
Private	nContBol := 1
Private aNotas   := {}
Public aNFEnv    := {} // Variavel que armazena as notas a enviar
	...
	If !lImpDir
		...
		While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
			...
			If (cAliasSF3)->(Eof())
					...
					If !Empty(aXML[nX][2])
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7") .And. !cCodRetNFE $ RetCodDene()
							...
							If Empty(cAviso) .And. Empty(cErro)
								...
								//³Adiciona as notas para envio/impressão de boletos³
								//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
								
								aAdd(aNFEnv, {aNotas[nX, 4], aNotas[nX, 5], aNotas[nX, 6], aNotas[nX, 7], aNotas[nX][02], aXML[nX]})
								
								//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄX¿
								//³Jean | 12/12/2013 | Tratativa para envio de xmls através da Neogrid - INICIO³
								//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄXÙ
								
								lEnvNeo := .F. 
								
								//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
								//³Avalia se o tipo de nota é saída e se o cliente/loja está setado para envio do xml pela Neogrid³
								//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
																
								If (aNotas[nX][02] == "S")
									DbSelectArea("SA1")
									SA1->(DbSetOrder(1))
									if DbSeek(xFilial("SA1") + aNotas[nX, 6] + aNotas[nX, 7])
										lEnvNeo := (SA1->A1_X_NEOG == "1")	
									EndIf
								EndIf
								...
								if lEnvNeo
									...
									//³Obtém o email do cliente/fornecedor para quem deverá enviar a NF
									//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
									
									cEmail := U_GetMailNF(aNotas[nX, 4], aNotas[nX, 5], aNotas[nX, 6], aNotas[nX, 7], aNotas[nX][02])
								
									//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
									//³Grava na tabela de envio de notas fiscais³
									//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
									
									DbSelectArea("Z35")
									Z35->(DbSetOrder(2))
									lFound := DbSeek(xFilial("Z35") + AllTrim(cEmpAnt) + AllTrim(cFilAnt) + aNotas[nX, 4] + aNotas[nX, 5])
									...
									Else
										...
										Z35->Z35_CODEMP := cEmpAnt
										Z35->Z35_CODFIL := cFilAnt
										Z35->Z35_CODNF  := aNotas[nX, 5]
										Z35->Z35_SERNF  := aNotas[nX, 4]
										Z35->Z35_TIPONF := aNotas[nX][02]
										Z35->Z35_ENVXML := "N"
										Z35->Z35_IDENT  := cIdent
										Z35->Z35_EMAIL  := cEmail
										Z35->(MsUnlock())
									EndIf
					      ...
					      EndIf
								...
								//³Jean | 12/12/2013 | Tratativa para envio de xmls através da Neogrid - FIM³
								//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
								
								//**********************************************//
								// Customizado FVSISTEMAS - Flavio - CDR Entrega Certa
								//**********************************************//
								if aNotas[nX][02]=="S" .And. (lGeraPDF)
								  
								  // Valida se gera estoque para enviar para o armazém
									lGeraEst := .F.
									SD2->(dbSetOrder(3))
									SD2->(MsSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA))
									While SD2->D2_FILIAL + SD2->D2_DOC + SD2->D2_SERIE == SF2->F2_FILIAL + SF2->F2_DOC + SF2->F2_SERIE //.And. !lGeraEst
										SF4->(dbSetOrder(1))
										...
										nPos := aScan(aPedXml, {|x| x[1] == SD2->D2_PEDIDO})
										if nPos == 0
								  ...
								  if lGeraEst								  
									  dbSelectArea("SA1")
										dbSetOrder(1)
										SA1->(MsSeek(xFilial("SA1")+ SF2->F2_CLIENTE + SF2->F2_LOJA,.T.))										
									  ...
									  if !ExistDir("\nfe\")
										...
										oDanfePDF:SetMargin(00,00,00,00)
										oDanfePDF:linjob   :=.F.
										
										oDanfePDF:cPathPDF := cPathLoc
										oDanfePDF:SetDevice(IMP_PDF)
										oDanfePDF:SetViewPDF(.F.)
										//oDanfePDF:lServer := .F.
										
										ImpDet(@oDanfePDF,oNFe,cAutoriza,cModalidade,oNfeDPEC,cCodAutDPEC,aXml[nX][6],aXml[nX][7],aNotas[nX],lImpSimp)
										oDanfePDF:Print()
										FreeObj(oDanfePDF)
										
										CpyT2S(cPathLoc + Trim(cEmpAnt) + Trim(SF2->F2_FILIAL) + Trim(SF2->F2_DOC) + ".pdf", "\nfe\", .F.)
										cXmlW := aXml[nX, 2]
										// caso retornado, monta o xml com a autorização de uso
										if (Len(aXml[nX]) == 11)
											cXmlW := '<?xml version="1.0" encoding="UTF-8"?><nfeProc xmlns="http://www.portalfiscal.inf.br/nfe" versao="3.10">' + ;
																 StrTran(cXmlW, '<?xml version="1.0" encoding="UTF-8"?>', '') + aXml[nX, 11] + '</nfeProc>';
												
										endIf
										MemoWrite(cTempPath + Trim(cEmpAnt) + Trim(SF2->F2_FILIAL) + Trim(SF2->F2_DOC) + ".xml", cXmlW)
										aAdd(aPDFXml, {cTempPath + Trim(cEmpAnt) + Trim(SF2->F2_FILIAL) + Trim(SF2->F2_DOC) + ".xml", ;
																	 cTempPath + Trim(cEmpAnt) + Trim(SF2->F2_FILIAL) + Trim(SF2->F2_DOC) + ".pdf", ;
																	 SF2->F2_DOC, SF2->F2_SERIE, SF2->F2_FILIAL, SF2->F2_CHVNFE, SA1->A1_CGC, ;
																	 DToS(SF2->F2_EMISSAO), if (Empty(SF2->F2_VEND1), "0", SubStr(SF2->F2_VEND1, 2)), ;
																	 SF2->F2_VOLUME1, SF2->F2_VALFAT, SF2->F2_PBRUTO})
									endIf
								endIf
				...
				// Customizado FVSISTEMAS - Flavio - CDR Entrega Certa
				//**********************************************//
				if lGeraPDF .And. Len(aPDFXml) > 0
					cMsgInt := U_RJENVCDR(aPDFXml, .F., aPedXml, aItPedXml)
					if !Empty(cMsgInt)
...
/**CANTU-INICIO**/
// Quando for compilar para o siga antigo, deixar comentado e depois voltar ao normal
For nX := 1 to len(aNFEnv)
	if (!lEnvPdfJob)
		U_SendXml(aNFEnv[nX, 1], aNFEnv[nX, 2], aNFEnv[nX, 3], aNFEnv[nX, 4], aNFEnv[nX, 5], aNFEnv[nX, 6])
	EndIf
	// Adriano - Aqui Boletos
Next
/**CANTU-FIM**/
RestArea(aArea)
Return(.T.)
...
PRIVATE oFont10N   := TFontEx():New(oDanfe,"Times",08,08,.T.,.T.,.F.)// 1
PRIVATE oFont07N   := TFontEx():New(oDanfe,"Times",06,06,.T.,.T.,.F.)// 2
PRIVATE oFont07    := TFontEx():New(oDanfe,"Times",06,06,.F.,.T.,.F.)// 3
PRIVATE oFont08    := TFontEx():New(oDanfe,"Times",07,07,.F.,.T.,.F.)// 4
PRIVATE oFont08N   := TFontEx():New(oDanfe,"Times",06,06,.T.,.T.,.F.)// 5
PRIVATE oFont09N   := TFontEx():New(oDanfe,"Times",08,08,.T.,.T.,.F.)// 6
PRIVATE oFont09    := TFontEx():New(oDanfe,"Times",08,08,.F.,.T.,.F.)// 7
PRIVATE oFont10    := TFontEx():New(oDanfe,"Times",09,09,.F.,.T.,.F.)// 8
PRIVATE oFont11    := TFontEx():New(oDanfe,"Times",10,10,.F.,.T.,.F.)// 9
PRIVATE oFont12    := TFontEx():New(oDanfe,"Times",11,11,.F.,.T.,.F.)// 10
PRIVATE oFont11N   := TFontEx():New(oDanfe,"Times",10,10,.T.,.T.,.F.)// 11
PRIVATE oFont18N   := TFontEx():New(oDanfe,"Times",17,17,.T.,.T.,.F.)// 12 
PRIVATE OFONT12N   := TFontEx():New(oDanfe,"Times",11,11,.T.,.T.,.F.)// 12 
...
aTotais[02] := Transform(Val(oTotal:_ICMSTOT:_vICMS:TEXT),"@e 9,999,999,999.99")
...
If ( MV_PAR04 == 1 )
	...
	If MsSeek(xFilial("SF1")+aNota[5]+aNota[4]+aNota[6]+aNota[7]) .And. SF1->(FieldPos("F1_FIMP"))<>0
		If SF1->F1_TIPO <> "D"
		  	aTotais[10] := 	Transform(Val(oTotal:_ICMSTOT:_vIPI:TEXT),"@e 9,999,999,999.99")
...
For nZ := 1 To nLenDet
	...
	aAux := {}
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1], 5/* TamSX3("D2_QUANT")[2]*/ )))) // Flavio - Deixado fixo 5 casas decimais
	...
	If "MT" $ Alltrim( Upper( SuperGetMv("MV_ESTADO") ) ) .And. ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		aAuxCom := {}
		AADD(aAuxCom, AllTrim(TransForm(nQtdCom,TM(nQtdCom,TamSX3("D2_QUANT")[1],/*TamSX3("D2_QUANT")[2]*/ 5))))
...
//Guilherme - 19/05/16 - Se houver complemento de endereço, então preenche nas informações adicionais
cCmpl	:= POSICIONE("SA1",1,xFilial("SA1")+cCliFor+cLojaCF,"A1_COMPLEM")
If !Empty(Alltrim(cCmpl))
	cAux := "Complemento do endereço: "+cCmpl
	While !Empty(cAux)
		aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	...
	*/
	// Flavio - 24/11/2011
	// Alterado para adicionar a quebra de linha quando existir os ..
	// cAux := stripTags(oNF:_InfNfe:_infAdic:_InfCpl:TEXT, .T.)
	//aMsgTemp := StrToKArr(oNF:_InfNfe:_infAdic:_InfCpl:TEXT, "..")      
	aMsgTemp := Separa(oNF:_InfNfe:_infAdic:_InfCpl:TEXT, ". ") //Gustavo -- 30/01/17	
	For nMsgTemp := 1 to Len (aMsgTemp)
		cAux := StrTran(aMsgTemp[nMsgTemp], '..', '.') //stripTags(oNF:_InfNfe:_infAdic:_InfCpl:TEXT, .T.)
		cAux := StrTran(cAux, '..', '.')
		While !Empty(cAux)
			aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
			cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
...
/**CANTU-INICIO**/
oDanfe:Say(028, 102, NoChar(oDestino:_XNome:TEXT,lConverte), oFont07:oFont)//Roberto - 02/04/2013 - Imprime o nome do cliente no canhoto.
/**CANTU-FIM**/
...
If lMv_Logod
	...
	//Gustavo Lattmann 29/10/2014 - Alterado para poder ter logo espefico por empresa e filial
...
If nfolha==1
	...
	Else
		...
		oDanfe:SayBitmap(052,010,cLogo,080,035)		
		/**CANTU-FIM**/
	EndIF
Endif
...
oDanfe:Say(095,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont08:oFont)
...
If Empty(cCodAutDPEC)
	If Empty(cChaveCont)
		oDanfe:Say(117,355,"Consulta de autenticidade no portal nacional da NF-e",oFont09:oFont)
		oDanfe:Say(127,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont08:oFont)
	Endif
Endif

If  !Empty(cCodAutDPEC)
	oDanfe:Say(117,355,"Consulta de autenticidade no portal nacional da NF-e",oFont11:oFont)
	oDanfe:Say(127,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont11:oFont)
Endif
...
/**CANTU-INICIO**/
//25/11/2014 Gustavo - Tratamento para imprimir ou não data de saída na Danfe
If SuperGetMv("MV_X_DTNFE",,"S") == "S"
...
/**CANTU-INICIO**/
// Flavio - Adicionado o frete de acordo com a configuração da tela do envio
cModFrete := aTransp[02]
/**CANTU-FIM**/
If cModFrete =="0"
...
// Flavio - FV Sistemas - Deixa fixo o tamanho das colunas, devido aos cálculos do protheus possuirem erro
//aTamCol := RetTamCol(aAuxCabec, aAux, oDanfe, oFont08:oFont, oFont08N:oFont)
aTamCol := {45, 180, 36, 16, 20, 14, 49, 43, 49, 40, 34, 25, 31, 21}
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			oDanfe:Say(058+nLinhavers,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont08N:oFont)
			
			oDanfe:Say(048+nLinhavers,355,"CHAVE DE ACESSO DA NF-E",oFont08N:oFont)
			nFontSize := 28
			...
			Else
				/**CANTU-INICIO**/
				oDanfe:SayBitmap(/*010+nLinhavers*/052,000,cLogo,080,035)
				//oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,096)
				/**CANTU-FIM**/
			EndIf
			
			If Empty(cChaveCont)
				oDanfe:Say(075+nLinhavers,355,"Consulta de autenticidade no portal nacional da NF-e",oFont08N:oFont)
				oDanfe:Say(085+nLinhavers,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont08N:oFont)
			Endif
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		oDanfe:Say(058,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont08N:oFont)
		...
		Else
			...
			oDanfe:SayBitmap(010+nLinhavers,000,cLogo,080,035)
			/**CANTU-FIM**/
		EndIf
		
		If Empty(cChaveCont)
			oDanfe:Say(075,355,"Consulta de autenticidade no portal nacional da NF-e",oFont08:oFont)
			oDanfe:Say(085,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont08:oFont)
		Endif
	...
	Else
		...
		*/
		...
		EndIf          
		oDanfe:Say(nLinha, nAuxH2 - 8, aAux[1][7][nY], oFont08:oFont,,,,PAD_RIGHT) // QUANT
		nAuxH += aTamCol[7]-10
		If Empty(aAux[1][6][nY]) // UN
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 + 1, aAux[1][8][nY], oFont08:oFont,,,,PAD_RIGHT) // V UNITARIO
		nAuxH += aTamCol[8]
		If Empty(aAux[1][6][nY]) // UN
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 1, aAux[1][9][nY], oFont08:oFont,,,,PAD_RIGHT) // V. TOTAL
		nAuxH += aTamCol[9]
		If Empty(aAux[1][6][nY]) // UN
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 1, aAux[1][10][nY], oFont08:oFont) // BC. ICMS
		nAuxH += aTamCol[10]
		If Empty(aAux[1][6][nY]) // UN
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 1, aAux[1][11][nY], oFont08:oFont) // V. ICMS
		nAuxH += aTamCol[11]
		If Empty(aAux[1][6][nY]) // UN
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 1, aAux[1][12][nY], oFont08:oFont) // V.IPI
		nAuxH += aTamCol[12]
		nAuxH2 := nAuxH + ((aTamCol[13] - 2) - RetTamTex(aAux[1][13][nY], oFont08:oFont, oDanfe)) + 2
		oDanfe:Say(nLinha, nAuxH2 + 1, aAux[1][13][nY], oFont08:oFont) // A.ICMS
		nAuxH += aTamCol[13]
		nAuxH2 := nAuxH + ((aTamCol[14] - 2) - RetTamTex(aAux[1][14][nY], oFont08:oFont, oDanfe)) + 2
		oDanfe:Say(nLinha, nAuxH2 + 1, aAux[1][14][nY], oFont08:oFont) // A.IPI
...
If lMensagens
	...
	oDanfe:Say(048,355,"CHAVE DE ACESSO DA NF-E",oFont08N:oFont)
	nFontSize := 28
	oDanfe:Code128C(036,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
	...
	Else
		/**CANTU-INICIO**/
		//oDanfe:SayBitmap(000,000,cLogo,095,096)
		oDanfe:SayBitmap(020,000,cLogo,080,035) // Guilherme, alterado para nLinCalc pq nao estava encontrando o registro		
		/**CANTU-FIM**/
	EndIf
	
	If Empty(cChaveCont)
		oDanfe:Say(075,355,"Consulta de autenticidade no portal nacional da NF-e",oFont08N:oFont)
		oDanfe:Say(085,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont08N:oFont)
	Endif
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	If nAux < 603
		/**CANTU-INICIO**/
		//aTamCol[2] += 603 - nAux
		// Jean - 17/11/2012 - Tratamento para dar mais espaço para campo quantidade.
		If (nAux + 20) < 603
			aTamCol[2] += (603 - 10 - nAux)
			aTamCol[7] += 10
		Else
			aTamCol[2] += 603 - nAux
		EndIf
	...
	//³Jean - 17/11/2012 - Removido 10 pixels da coluna valor unitário e colocado na coluna valor total³
...
/**CANTU-INICIO**/
// Função criada apenas para ser executada via job, pois demora muito para gerar o PDF
User Function ImpDet(oDanfe,oNfe,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
...
For nZ := 1 To nLenDet
	...
	// Tratamento para quebrar os digitos dos valores
	aAux := {}
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1], 5 /*TamSX3("D2_QUANT")[2]*/))))
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2]))))
	AADD(aAux, AllTrim(TransForm(nVTotal,TM(nVTotal,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2]))))
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GetMailNF ºAutor  ³Jean Carlos Saggin  º Data ³  02/04/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Função criada para retornar e-mail do cliente ou fornec.   º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ Faturamento                                                º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
*/
*--------------------------------------------------------------*
User Function GetMailNF(cSerie, cNota, cCliFor, cLoja, cTipo)
*--------------------------------------------------------------*
Local cEmail := ""
Local lCliente := .T.
