...
TRCell():New(oSection,'VALOR'		,cAliasTRB,STR0049+CRLF+STR0044,cPictVL,aSizeVL[1],/*lPixel*/,/*{|| code-block de impressao }*/,,,"RIGHT")
TRCell():New(oSection,'VALORR'		,cAliasTRB,STR0049+CRLF+STR0050,cPictVL,aSizeVL[1],/*lPixel*/,/*{|| code-block de impressao }*/,,,"RIGHT")
...
//³ Ajusta as perguntas para Custo Unificado                     |
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If lCusUnif .And. ((mv_par01==1) .Or. !(mv_par04=='**') .Or. !(mv_par05=='**') .Or. nOrdem==5)
...
If lRet
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	TRPosition():New(oSection,"SB1",1,{|| If(mv_par01==3 .And. xFilial("SB1")#Space(2) .And. xFilial("SB2")#Space(2),(cAliasTRB)->FILIAL+(cAliasTRB)->CODIGO,xFilial("SB1")+(cAliasTRB)->CODIGO)})
	...
	If mv_par01 == 2
		...
		EndIf
	ElseIf mv_par01 == 3
		oSection:Cell("B2_FILIAL"):SetValue('**')
		If !(nOrdem == 5)
			oSection:Cell("B2_LOCAL"):SetValue('**')
		EndIf
	EndIf
	...
	If lAglutLoc .Or. lAglutFil
		...
		Do Case
			Case (nOrdem == 1)
				...
				EndIf
				cSeek += (cAliasSB2)->B2_COD+IIf(lAglutFil,"",(cAliasSB2)->B2_FILIAL)+IIf(lAglutLoc,"",(cAliasSB2)->B2_LOCAL)
			Case (nOrdem == 2)
			...
			Case (nOrdem == 3)
				If (mv_par17 == 1)
					cSeek += IIf(mv_par22==1,(cAliasSB1)->B1_SEGUM,(cAliasSB1)->B1_UM)
				EndIf
				cSeek += (cAliasSB1)->B1_DESC+(cAliasSB2)->B2_COD+IIf(lAglutFil,"",(cAliasSB2)->B2_FILIAL)+IIf(lAglutLoc,"",(cAliasSB2)->B2_LOCAL)
			Case (nOrdem == 4)
			...
			Case (nOrdem == 5)
				If (mv_par17 == 1)
					cSeek += IIf(mv_par22==1,(cAliasSB1)->B1_SEGUM,(cAliasSB1)->B1_UM)
				EndIf
				cSeek += (cAliasSB2)->B2_COD+IIf(lAglutFil,"",(cAliasSB2)->B2_FILIAL)
			OtherWise
				If (mv_par17 == 1)
					cSeek += IIf(mv_par22==1,(cAliasSB1)->B1_SEGUM,(cAliasSB1)->B1_UM)
				EndIf
				cSeek += (cAliasSB2)->B2_COD+IIf(lAglutLoc,"",(cAliasSB2)->B2_LOCAL)
		EndCase
	EndIf
...
Local cPicVal	:= PesqPict("SB2","B2_VATU"+If(mv_par22 == 1,"2","1") ,15)
	...
	If ( (mv_par14 == 1)  .Or.;
		...
		If lPrintDET
			...
			If lPrintCAB .Or. lPrintOUT
				...
				@ Li,046 + (nCOL1+60) PSay aPrnDET[ DET_UM   ]
				
				lPrintCAB := .F. ; lPrintOUT := .F.
			EndIf
			
			@ Li,049 + (nCOL1+60) PSay aPrnDET[ DET_FL   ]
			@ Li,052 + (nCOL1+60) PSay aPrnDET[ DET_ALMX ]
	
			@ Li,054 + (nCOL1+60) PSay aPrnDET[ DET_SALD ] Picture cPicture
			@ Li,070 + (nCOL1+60) PSay aPrnDET[ DET_EMPN ] Picture cPicture
			@ Li,085 + (nCOL1+60) PSay aPrnDET[ DET_DISP ] Picture cPicture
			@ Li,100 + (nCOL1+60) PSay aPrnDET[ DET_VEST ] Picture cPicVal
			@ Li,117 + (nCOL1+60) PSay aPrnDET[ DET_VEMP ] Picture cPicVal
	        If mv_par23 == 1
		        @ Li,136 + (nCOL1+60) PSay aPrnDET[ DET_DEAL ]
	        EndIf
	...
	If lPrintTOT
		...
		If lT .OR. lT1
			...
			If nORDEM <> 5 
				...
				@ Li,054 + (nCOL1+60) PSay aTotUM[ ACM_SALD ] Picture cPicture
				@ Li,070 + (nCOL1+60) PSay aTotUM[ ACM_EMPN ] Picture cPicture
				@ Li,085 + (nCOL1+60) PSay aTotUM[ ACM_DISP ] Picture cPicture
				@ Li,100 + (nCOL1+60) PSay aTotUM[ ACM_VEST ] Picture cPicVal
				@ Li,117 + (nCOL1+60) PSay aTotUM[ ACM_VEMP ] Picture cPicVal
			...
			Else
				If lT1  
					...
					@ Li,054 + (nCOL1+60) PSay aTotUM1[ ACM_SALD ] Picture cPicture
					@ Li,070 + (nCOL1+60) PSay aTotUM1[ ACM_EMPN ] Picture cPicture
					@ Li,085 + (nCOL1+60) PSay aTotUM1[ ACM_DISP ] Picture cPicture
					@ Li,100 + (nCOL1+60) PSay aTotUM1[ ACM_VEST ] Picture cPicVal
					@ Li,117 + (nCOL1+60) PSay aTotUM1[ ACM_VEMP ] Picture cPicVal
		...
		If nOrdem == 5 .And. mv_par01 == 1 .And. cLocal != FIELD->LOCAL
			If nOrdem == 5
				...
				+ cLocal
				@ Li,054 + (nCOL1+60) PSay aTotAMZ[ ACM_SALD ] Picture cPicture
				@ Li,070 + (nCOL1+60) PSay aTotAMZ[ ACM_EMPN ] Picture cPicture
				@ Li,085 + (nCOL1+60) PSay aTotAMZ[ ACM_DISP ] Picture cPicture
				@ Li,100 + (nCOL1+60) PSay aTotAMZ[ ACM_VEST ] Picture cPicVal
				@ Li,117 + (nCOL1+60) PSay aTotAMZ[ ACM_VEMP ] Picture cPicVal
				Li++
		...
		If (((nOrdem == 2) .Or. (nOrdem == 4)) .And. ;
			...
			@ Li,016 PSay OemToAnsi(STR0016)+If( (nOrdem == 2),OemToAnsi(STR0017),OemToAnsi(STR0018))+" : "+aTotORD[ ACM_CODE ]   //"Total do "###"Tipo"###"Grupo"
	 	   	@ Li,054 + (nCOL1+60) PSay aTotORD[ ACM_SALD ] Picture cPicture
		   	@ Li,070 + (nCOL1+60) PSay aTotORD[ ACM_EMPN ] Picture cPicture
		   	@ Li,085 + (nCOL1+60) PSay aTotORD[ ACM_DISP ] Picture cPicture
		   	@ Li,100 + (nCOL1+60) PSay aTotORD[ ACM_VEST ] Picture cPicVal
		   	@ Li,117 + (nCOL1+60) PSay aTotORD[ ACM_VEMP ] Picture cPicVal
...
If nTotValSal + nTotValRPR + nTotValRes + nTotValEst + nTotValEmp # 0
	...
	@ Li,054 + (nCOL1+60) PSay nTotValSal Picture cPicture
	@ Li,070 + (nCOL1+60) PSay nTotValRPR Picture cPicture
	@ Li,085 + (nCOL1+60) PSay nTotValRes Picture cPicture
	@ Li,100 + (nCOL1+60) PSay nTotValEst Picture cPicVal
	@ Li,117 + (nCOL1+60) PSay nTotValEmp Picture cPicVal
EndIf
