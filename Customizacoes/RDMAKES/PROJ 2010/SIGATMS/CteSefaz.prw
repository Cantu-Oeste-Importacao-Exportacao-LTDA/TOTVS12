			...
			If (!(cAliasDT6)->(Eof())) // (01)
				...
				While !(cAliasDT6)->(Eof()) // (02)
					...
					If	( Empty((cAliasDT6)->DT6_CLICON) .And. Empty((cAliasDT6)->DT6_CLIDPC) ) .Or. ;
						...
						If (((cAliasDT6)->DT6_CLIREM = (cAliasDT6)->DT6_CLIDEV) .And.;
							((cAliasDT6)->DT6_LOJREM = (cAliasDT6)->DT6_LOJDEV))
							// Guilherme 03/06/14 - tratar EX
							If AllTrim((cAliasDT6)->REM_UF) == "EX"
								cCNPJToma 	:= '00000000000000'
						...
						ElseIf (((cAliasDT6)->DT6_CLIDES = (cAliasDT6)->DT6_CLIDEV) .And.;
								((cAliasDT6)->DT6_LOJDES = (cAliasDT6)->DT6_LOJDEV))
						   	// Guilherme 03/06/14 - tratar EX
							If AllTrim((cAliasDT6)->DES_UF) == "EX"
								cCNPJToma 	:= '00000000000000' 
					...
					ElseIf (((cAliasDT6)->DT6_CLICON = (cAliasDT6)->DT6_CLIDEV) .And.;
						...
						// Guilherme 03/06/14 - tratar EX
						If AllTrim((cAliasDT6)->CON_UF) == "EX"
							cCNPJToma 	:= '00000000000000'			
						...
						//Guilherme 03/06/14 tratar a impressao quando o Tomador for EX para preencher com zeros o CNPJ/CPF
						If (AllTrim((cAliasDT6)->CON_TPPESSOA) == "J")
							If AllTrim((cAliasDT6)->CON_UF) == "EX"
								cString += '<CNPJ>00000000000000</CNPJ>' //Guilherme 03/06/14
							Else	
								cString += '<CNPJ>' + AllTrim((cAliasDT6)->CON_CNPJ) + '</CNPJ>'
							EndIF	
						Else
							If AllTrim((cAliasDT6)->CON_UF) == "EX"
								cString += '<CPF>00000000000</CPF>'
							Else	
					...
					ElseIf (((cAliasDT6)->DT6_CLIDPC = (cAliasDT6)->DT6_CLIDEV) .And.;
						...
						If (AllTrim((cAliasDT6)->DPC_TPPESSOA) == "J")
							If Alltrim((cAliasDT6)->DPC_UF) == "EX"  //Guilherme 03/06/14
					...
					If	!AllTrim((cAliasDT6)->DT6_DOCTMS) == StrZero( 8, Len(DT6->DT6_DOCTMS)) .And.;
						...
						//guilherme 03/06/14 - Verificar se é operação nacional ou internaciona par definir a apólice do seguro no caso de ser a RJU a responsável pelo seguro
	    				lExterior := .F.
	    				If ((cAliasDT6)->DES_UF == 'EX' .OR. (cAliasDT6)->(REM_UF) == 'EX' )
								lExterior := .T.							    				
	    				EndIF
						...
						If !Empty(cForSeg) .And. (Empty(cNomSeg) .And. Empty(cApolice) .And. Empty(cRespSeg)) .And. lForApol
							...
							If (cAliasSeg)->(!EoF())
								...
								cNomSeg   := (cAliasSeg)->A2_NOME 
								If lExterior
									cApolice := Alltrim(SuperGetMV("MV_X_APOEX"))
									//cApolice  := (cAliasSeg)->A2_APOLICE 
								Else
									cApolice := Alltrim(SuperGetMV("MV_X_APONA"))
								EndIf
								//cApolice  := (cAliasSeg)->A2_APOLICE
							EndIf
							(cAliasSeg)->(DbCloseArea())
