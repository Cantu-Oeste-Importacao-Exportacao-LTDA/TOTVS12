...
cChave := MDFeChave( aUF[aScan(aUF,{|x| x[1] == SM0->M0_ESTCOB})][02],;
...
/*Elementos RNTRC e CIOT não gerados*/
		...
		If !Empty(TRB->TRB_MARCA)
			...
			While TRB->(!Eof()) .and. cCodMun == TRB->TRB_CODMUN
				...
				If !Empty(TRB->TRB_MARCA)
					...
					if substr(TRB->TRB_CHVNFE,35,1) <> '1'
						cString += '<SegCodBarra>'+TRB->TRB_CHVNFE+'</SegCodBarra>'
					endif		
					cString += '</infNFe>'					
				EndIf
				TRB->(dbSkip())
...
Static Function MDFeInfAdic()
	...
	If !Empty(cInfFsc) .or. !Empty(cInfCpl)
		cString += "<infAdic>"
		If !Empty(cInfFsc)
			cString += NfeTag("<infAdFisco>",ConvType(cInfFsc))
		EndIf
		If !Empty(cInfCpl)
			cString += NfeTag("<infCpl>",ConvType(cInfCpl))
		EndIf		
		cString += "</infAdic>"
	EndIf
